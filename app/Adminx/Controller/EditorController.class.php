...
class EditorController extends AdminController {
	...
	public function UploadImage(){
		...
		$upload->rootPath = '.'.C('UPLOAD_PATH').'images/';        //上传保存到什么地方？路径建议大家已主文件平级目录或者平级目录的子目录来保存
		$upload->autoSub = true;
    ...
    public function UploadFile(){
    	//上传图片框中的描述表单名称，
		$upload = new \Think\Upload();// 实例化上传类
		$upload->maxSize = C('file_size')*1024*1024;  //是指上传文件的大小，默认为-1,不限制上传文件大小bytes		
		$upload->rootPath = '.'.C('UPLOAD_PATH').'files/';        //上传保存到什么地方？路径建议大家已主文件平级目录或者平级目录的子目录来保存
		$upload->autoSub = true;
		$upload->replace=true;     //如果存在同名文件是否进行覆盖
    ...
    public function Scraw(){
		...
		if ( $Controller == "tmpImg" ) {
        	...
        	$upload->rootPath = '.'.C('UPLOAD_PATH').'tmp/';  
