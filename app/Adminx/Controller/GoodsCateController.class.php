...
class GoodsCateController extends AdminController {
	...
	public function index() {
		if (IS_POST) {
			...
			if(empty($cityID)){die;}
			$map['cityID']=$cityID;
			$list = M('GoodsCate')->where($map)->field("id,name,comm,fid,path,sort")->order('path,id asc')->select();
 	...
 	public function add(){
 		if($_POST){
			...
			if($data = $cate->create()){
 			...
 			$fid = I('get.id');
 			$path = I('get.path');
			$cityID=I('get.cityID',9);
			$map['cityID']=$cityID;
			$artClass = M('GoodsCate');
			$cate = $artClass->where($map)->field("id,fid,name,path")->order('path')->select();
	...
	public function edit(){
		if($_POST){
			...
			$id = I('get.id');
			if(isset($id)){
				$list=M('GoodsCate')->where('id='.$id)->find();
				if($list){
					$map['cityID']=$list['cityID'];
					$cate = M('GoodsCate')->where($map)->field("id,fid,name,path")->order('path')->select();
					foreach ($cate as $key => $value) {
						$count = count(explode('-', $value['path'])) - 3;
						$cate[$key]['count'] = $count;
					}
