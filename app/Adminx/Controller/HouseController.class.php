...
class HouseController extends AdminController {
    ...
    public function add(){
        if($_POST){
            ...
            if ($data = $obj->create()) {
                if ($data['facility']) {
                    $data['facility'] = implode(",",$data['facility']);
                }
                ...
                if ($list = $obj->add($data)) {                 
                    ...
                    $this->success('操作成功',U('House/index'));
                } else {
                    $this->error('操作失败');
                }
            } else {
            ...
            $sheshi = M("OptionItem")->where(array('cate'=>7))->select();
            ...
            if (!$_SESSION['administrator']) {
                ...
                $map['fid'] = 0;
                $map['cityID'] = $this->user['cityID'];
                $subway = M('Subway')->where($map)->select();
    ...
    public function edit() {
        if ($_POST) {
            ...
            if (!$list) {
                ...
                if ($list['facility']!='') {
                    $list['facility'] = explode(",",$list['facility']);
                }
    ...
    public function getSubway(){
        ...
        $map['cityID'] = $cityID;
        $map['fid'] = $fid;
        $list = M('Subway')->field('id,name')->where($map)->select();
        if($list){
