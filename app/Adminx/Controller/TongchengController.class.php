...
class TongchengController extends AdminController {
		...
		if($_POST){
	        ...
	        if ($data = $obj->create()) {
            	...
            	if ($image) {
		            ...
		            $data['thumb_s'] = $this->getThumb($data['thumb'], 150, 150);
		            $data['thumb_b'] = $this->getThumb($data['thumb'], 600, 300);
		        }else{
		            $data['image'] = '';
		        }
	            if ($list = $obj->add($data)) {	            	
	            	...
	            	$this->success('操作成功',U('Tongcheng/index'));
	            } else {
	                $this->error('操作失败');
	            }
	        } else {
